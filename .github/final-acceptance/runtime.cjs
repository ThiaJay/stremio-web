...
const openVideo = async (page, episode = 1, take = 1, bingeGroup = 'runtime-fixture') => {
    await page.evaluate(async ({ base, episode, take, bingeGroup }) => {
    ...
    await context.route('**/*', async route => {
        ...
        if (route.request().resourceType() === 'script') {
            return route.fulfill({
                ...
                body: '// Optional external SDK omitted from isolated acceptance run.'
            });
        }
...
(async () => {
    ...
    for (const [engine, launcher] of [['chromium', chromium], ['firefox', firefox]]) {
        ...
        await runCase(browser, engine, 'credits-seek-current-media', async page => {
            await settings(page, 'ask', 0, true);
            await openVideo(page);
            await at(page, 101);
            await segmentKind(page, 'outro');
            const before = page.url();
            const selectedBefore = await page.evaluate(async () => (await window.core.getState('player')).selected.streamRequest.path.id);
            ...
            const selectedAfter = await page.evaluate(async () => (await window.core.getState('player')).selected.streamRequest.path.id);
            assert.equal(selectedBefore, 'tt9999999:1:1');
        ...
        await runCase(browser, engine, 'autoplay-enabled-direct-continuation', async page => {
            ...
            await openVideo(page);
            await at(page, 119.4, false);
            await page.waitForFunction(async () => {
                const player = await window.core.getState('player');
                return player.selected?.streamRequest?.path?.id === 'tt9999999:1:2';
            }, null, { timeout: 20000 });
            assert(page.url().includes('/player/'));
        });
        await runCase(browser, engine, 'autoplay-disabled-does-not-advance', async page => {
            await settings(page, 'ask', 30000, false);
            await openVideo(page);
            const before = page.url();
            await at(page, 119.4, false);
            await page.waitForFunction(() => document.querySelector('video').ended === true, null, { timeout: 20000 });
            await page.waitForTimeout(500);
            const player = await page.evaluate(async () => window.core.getState('player'));
            assert.equal(player.selected.streamRequest.path.id, 'tt9999999:1:1');
            assert.equal(page.url(), before);
        });
        await runCase(browser, engine, 'autoplay-missing-direct-continuation-fails-closed', async page => {
            await settings(page, 'ask', 30000, true);
            await openVideo(page, 1, 1, 'unmatched-binge-group');
            const before = page.url();
            await at(page, 119.4, false);
            await page.waitForFunction(() => document.querySelector('video').ended === true, null, { timeout: 20000 });
            await page.waitForTimeout(500);
        ...
        if (engine === 'chromium') await runCase(browser, engine, 'tv-remote-intro-and-credits', async page => {
            await page.setViewportSize({ width: 1920, height: 1080 });
            await settings(page, 'ask', 0, true);
            await openVideo(page);
            await at(page, 12);
            await segmentKind(page, 'intro');
            let button = page.getByRole('button', { name: 'Skip Intro', exact: true });
            await button.waitFor(); await button.focus(); await page.keyboard.press('Enter');
            await page.waitForFunction(() => document.querySelector('video').currentTime >= 20);
            await at(page, 101);
            await segmentKind(page, 'outro');
            button = page.getByRole('button', { name: 'Skip Credits', exact: true });
            await button.waitFor(); await button.focus(); await page.keyboard.press('Enter');
            await page.waitForFunction(() => document.querySelector('video').currentTime >= 110);
            const model = await page.evaluate(async () => window.core.getState('player'));
            assert.equal(model.selected.streamRequest.path.id, 'tt9999999:1:1');
        });

        await runCase(browser, engine, 'consumed-recap-survives-intro-and-rewind', async page => {
            await settings(page, 'ask', 0, true);
            await openVideo(page);
            await at(page, 3);
            await segmentKind(page, 'recap');
            let button = page.getByRole('button', { name: 'Skip Recap', exact: true });
            await button.waitFor(); await button.click();
            await page.waitForFunction(() => document.querySelector('video').currentTime >= 7);
            await at(page, 12);
            await segmentKind(page, 'intro');
            button = page.getByRole('button', { name: 'Skip Intro', exact: true });
            await button.waitFor(); await button.click();
            await page.waitForFunction(() => document.querySelector('video').currentTime >= 20);
            await at(page, 3);
            await segmentKind(page, 'recap');
            const state = await page.evaluate(async () => (await window.core.getState('player')).skipSegment);
            assert.equal(state.kind, 'recap');
